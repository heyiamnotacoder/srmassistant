...
Screening happens BEFORE extraction: it triages hundreds-to-thousands of citation records into `exclude` / `maybe` / `include`, so only plausible studies proceed to full-text retrieval and the extraction workflow (`prompts/orchestrator.md`).
...
1. Read every file in `protocol/` (PROSPERO application, PICO notes, protocol drafts). If empty, stop and ask the human for their protocol documents.
2. Write `screening/criteria.json`:
...
3. **Present the criteria to the human and get explicit confirmation.** Never screen against unconfirmed criteria.
